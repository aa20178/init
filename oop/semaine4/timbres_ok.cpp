...
public:
    ...
    double vente() const //est sa valeur faciale, si le timbre a moins que cinq ans ; sinon,il  vaut  la  valeur  faciale  multipliée  par  2.5  fois  l’âge  du  timbre
    {
        if (age() < 5)
        {
...
{
    os << "Timbre rare (" << obj.nb_exemplaires() << " ex.) ";
    obj.afficher(os);
    return os;
}
